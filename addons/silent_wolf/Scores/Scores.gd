...
func get_scores(maximum: int = 10, ldboard_name: String = "main", period_offset: int = 0) -> Node:
	...
	latest_max = maximum
	var request_url = (
		"https://api.silentwolf.com/get_scores/"
		+ str(SilentWolf.config.game_id)
		+ "?max="
		+ str(maximum)
		+ "&ldboard_name="
		+ str(ldboard_name)
...
func get_scores_by_player(player_name: String, maximum: int = 10, ldboard_name: String = "main", period_offset: int = 0) -> Node:
	if player_name == null:
		...
		latest_max = maximum
		var request_url = (
			"https://api.silentwolf.com/get_scores_by_player/"
			+ str(SilentWolf.config.game_id)
			+ "?max="
			+ str(maximum)
			+ "&ldboard_name="
			+ str(ldboard_name.uri_encode())
			+ "&player_name="
			+ str(player_name.uri_encode())
			+ "&period_offset="
			+ str(period_offset)
		)
...
func get_top_score_by_player(player_name: String, maximum: int = 10, ldboard_name: String = "main", period_offset: int = 0) -> Node:
	if player_name == null:
		...
		latest_max = maximum
		var request_url = (
			"https://api.silentwolf.com/get_top_score_by_player/"
			+ str(SilentWolf.config.game_id)
			+ "?max="
			+ str(maximum)
			+ "&ldboard_name="
...
func get_scores_around(score, scores_to_fetch = 3, ldboard_name: String = "main") -> Node:
	...
	#latest_max = maximum
	var request_url = (
		"https://api.silentwolf.com/get_scores_around/"
		+ str(SilentWolf.config.game_id)
		+ "?scores_to_fetch="
		+ str(scores_to_fetch)
		+ "&ldboard_name="
